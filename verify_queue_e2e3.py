#!/usr/bin/env python3
"""Final E2E queue test over Valkey :6380 with worker restart."""
...
def main() -> None:
    ...
    print("[1] deploy test job + regenerate autoload")
    print(run(JOB))

    print("\n[2] restart queue worker (svc_queue.sh auto-restarts in 5s)")
    print(run("pkill -f 'artisan queue:work' ; sleep 12 ; pgrep -af 'artisan queue:work' | grep -v grep | head -1 || echo NO-WORKER-YET"))
