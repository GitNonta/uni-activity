...
<div class="map-explorer-wrapper">
    ...
    <div id="gmapNavBanner" class="gmap-nav-banner" style="display:none;">
        ...
        <div class="gmap-nav-actions-group">
            <button type="button" class="gmap-nav-action-btn active" id="btnNavVoice" onclick="toggleNavVoice()" title="เปิด/ปิดเสียงนำทาง">
                <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.536 8.464a5 5 0 010 7.072m2.828-9.9a9 9 0 010 12.728M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z"/>
                </svg>
            </button>
            <button type="button" class="gmap-nav-action-btn" id="btnNavSteps" onclick="toggleNavStepsDrawer()" title="ดูขั้นตอนทั้งหมด">
                <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h16M4 18h16"/>
                </svg>
            </button>
            <button type="button" class="gmap-nav-action-btn" id="btnNavSim" onclick="toggleNavSimulation()" title="จำลองการเดินทาง (Demo)">
                <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="map-canvas-container">
        ...
        <div class="gmap-fab-column">
            {{-- Route Planner FAB --}}
            <button type="button" class="gmap-fab gmap-fab-route" id="btnOpenRoutePlanner" onclick="openRoutePlannerModal()" title="วางแผนเส้นทาง / ค้นหาเส้นทาง">
                <svg width="20" height="20" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {{-- ── 6. Step-by-Step Turn-by-Turn Directions Drawer ── --}}
        <div id="gmapStepsDrawer" class="gmap-steps-drawer" style="display:none;">
            <div class="gmap-drawer-header">
                <div class="flex items-center gap-2">
                    <div class="gmap-title-icon" style="background:#10b981;color:#fff;">
                        <svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"/>
                        </svg>
                    </div>
                    <div>
                        <div class="font-bold" style="font-size:0.95rem;">ขั้นตอนการเดินทาง</div>
                        <div class="text-xs text-muted" id="stepsSummaryLabel">คำแนะนำการเลี้ยวตามเส้นทาง</div>
                    </div>
                </div>
                <button type="button" class="gmap-icon-btn" onclick="toggleNavStepsDrawer()">✕</button>
            </div>
            <div id="gmapStepsList" class="gmap-drawer-body">
                <div style="padding:2rem;text-align:center;color:#94a3b8;">ยังไม่มีข้อมูลเส้นทาง</div>
            </div>
        </div>

        {{-- ── 7. Advanced Route Planner Modal (Origin & Destination Picker) ── --}}
        <div id="gmapRoutePlannerModal" class="gmap-route-planner-modal" style="display:none;">
            <div class="gmap-planner-header">
                <div class="flex items-center gap-2">
                    <div class="gmap-title-icon" style="background:#ea580c;color:#fff;">
                        <svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7"/>
                        </svg>
                    </div>
                    <div>
                        <div class="font-bold" style="font-size:0.95rem;">ค้นหาและวางแผนเส้นทาง</div>
                        <div class="text-xs text-muted">เลือกจุดเริ่มต้นและจุดหมายปลายทาง</div>
                    </div>
                </div>
                <button type="button" class="gmap-icon-btn" onclick="closeRoutePlannerModal()">✕</button>
            </div>
            <div class="gmap-planner-body">
                <div class="gmap-planner-points">
                    <div class="gmap-point-row">
                        <span class="gmap-point-dot dot-origin"></span>
                        <div style="flex:1;min-width:0;">
                            <label class="gmap-planner-label">จุดเริ่มต้น</label>
                            <select id="routeOriginSelect" class="gmap-planner-select">
                                <option value="gps">📍 ตำแหน่งปัจจุบันของคุณ (GPS)</option>
                            </select>
                        </div>
                    </div>
                    <div class="gmap-point-swap-wrapper">
                        <button type="button" class="gmap-point-swap-btn" onclick="swapPlannerPoints()" title="สลับจุดเริ่มต้นและจุดหมาย">
                            <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="gmap-point-row">
                        <span class="gmap-point-dot dot-dest"></span>
                        <div style="flex:1;min-width:0;">
                            <label class="gmap-planner-label">จุดหมายปลายทาง</label>
                            <select id="routeDestSelect" class="gmap-planner-select">
                                <option value="">🎯 เลือกสถานที่ หรือกิจกรรมปลายทาง...</option>
                            </select>
                        </div>
                    </div>
                </div>

                {{-- Planner Travel Modes --}}
                <div class="gmap-travel-modes-row" style="margin-top:12px;">
                    <button type="button" class="gmap-mode-chip active" data-planner-mode="drive" onclick="selectPlannerMode('drive', this)">
                        🚗 รถยนต์
                    </button>
                    <button type="button" class="gmap-mode-chip" data-planner-mode="moto" onclick="selectPlannerMode('moto', this)">
                        🛵 มอเตอร์ไซค์
                    </button>
                    <button type="button" class="gmap-mode-chip" data-planner-mode="walk" onclick="selectPlannerMode('walk', this)">
                        🚶 เดินเท้า
                    </button>
                    <button type="button" class="gmap-mode-chip" data-planner-mode="bike" onclick="selectPlannerMode('bike', this)">
                        🚲 จักรยาน
                    </button>
                </div>

                <div style="margin-top:16px;">
                    <button type="button" class="gmap-btn-start-nav w-full" onclick="executePlannedRoute()">
                        <svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M14 5l7 7m0 0l-7 7m7-7H3"/>
                        </svg>
                        <span>คำนวณและดูเส้นทาง</span>
                    </button>
                </div>
            </div>
        </div>
...
/* ── Navigation Actions Group in Header Banner ── */
.gmap-nav-actions-group {
    display: flex;
    align-items: center;
    gap: 6px;
    flex-shrink: 0;
...
.gmap-nav-action-btn:hover {
    background: rgba(255, 255, 255, 0.35);
    transform: scale(1.08);
}
.gmap-nav-action-btn.active {
    background: #ffffff;
    color: #15803d;
...
.gmap-fab-route:hover {
    color: #ffffff;
    background: #059669;
}

/* ── Step-by-Step Directions Drawer ── */
.gmap-steps-drawer {
    position: absolute;
    top: 74px;
    left: 14px;
...
.gmap-step-card:hover {
    background: #f8fafc;
}
.gmap-step-card.active-step {
...
/* ── Route Planner Modal Sheet ── */
.gmap-route-planner-modal {
    position: absolute;
    top: 74px;
    left: 14px;
    right: 14px;
...
.dot-origin { background: #0284c7; box-shadow: 0 0 0 3px rgba(2, 132, 199, 0.2); }
.dot-dest { background: #ef4444; box-shadow: 0 0 0 3px rgba(239, 68, 68, 0.2); }
.gmap-planner-label {
    display: block;
    font-size: 0.68rem;
    font-weight: 700;
...
.gmap-point-swap-btn:hover {
    transform: rotate(180deg);
    background: #f8fafc;
}

/* Dark Mode Extensions for Route Components */
html[data-theme="dark"] .gmap-steps-drawer,
html[data-theme="dark"] .gmap-route-planner-modal {
    background: #18181b !important;
    border-color: #27272a !important;
    color: #f4f4f5 !important;
}
html[data-theme="dark"] .gmap-step-card {
    border-bottom-color: #27272a !important;
}
html[data-theme="dark"] .gmap-step-card:hover {
    background: #27272a !important;
}
html[data-theme="dark"] .gmap-step-title {
    color: #f4f4f5 !important;
}
html[data-theme="dark"] .gmap-step-icon {
    background: #27272a !important;
}
html[data-theme="dark"] .gmap-point-row {
    background: #27272a !important;
    border-color: #3f3f46 !important;
}
html[data-theme="dark"] .gmap-planner-select {
    color: #f4f4f5 !important;
    background: #27272a !important;
}
html[data-theme="dark"] .gmap-point-swap-btn {
    background: #27272a !important;
    border-color: #3f3f46 !important;
    color: #fb923c !important;
}
...
(function() {
        ...
        try {
            ...
            if (data && data.code === 'Ok' && data.routes && data.routes.length > 0) {
                ...
                data.routes.forEach((r, idx) => {
                    ...
                    const steps = (r.legs && r.legs[0] && r.legs[0].steps) ? parseOSRMSteps(r.legs[0].steps, target.title) : [];
    ...
    function parseOSRMSteps(osrmSteps, destTitle) {
        if (!osrmSteps || osrmSteps.length === 0) return [];

        return osrmSteps.map((s, i) => {
            const maneuver = s.maneuver || {};
            const type = maneuver.type || 'continue';
            const modifier = maneuver.modifier || '';
            const name = s.name ? s.name : 'ถนนหลัก';
            ...
            if (type === 'depart') {
                instruction = `เริ่มต้นออกเดินทางบน ${name}`;
                iconSvg = `<svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>`;
            } else if (type === 'arrive') {
                instruction = `ถึงจุดหมายปลายทาง (${destTitle || 'จุดหมาย'})`;
                iconSvg = `<svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/></svg>`;
            } else if (modifier.includes('left')) {
                instruction = modifier.includes('slight') ? `เบี่ยงซ้ายเข้าสู่ ${name}` : (modifier.includes('sharp') ? `เลี้ยวหักศอกซ้ายเข้าสู่ ${name}` : `เลี้ยวซ้ายเข้าสู่ ${name}`);
                iconSvg = `<svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>`;
            } else if (modifier.includes('right')) {
                instruction = modifier.includes('slight') ? `เบี่ยงขวาเข้าสู่ ${name}` : (modifier.includes('sharp') ? `เลี้ยวหักศอกขวาเข้าสู่ ${name}` : `เลี้ยวขวาเข้าสู่ ${name}`);
                iconSvg = `<svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>`;
            } else if (modifier.includes('uturn')) {
                instruction = `กลับรถเข้าสู่ ${name}`;
                iconSvg = `<svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9"/></svg>`;
            } else if (type === 'roundabout') {
                instruction = `เข้าสู่วงเวียนมุ่งหน้า ${name}`;
                iconSvg = `<svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="9" stroke-width="2"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 7v5l3 3"/></svg>`;
            } else {
                instruction = `ตรงต่อไปบน ${name}`;
                iconSvg = `<svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 10l7-7m0 0l7 7m-7-7v18"/></svg>`;
            }

            const point = maneuver.location ? [maneuver.location[1], maneuver.location[0]] : null;

            return {
                index: i + 1,
                instruction: instruction,
                distText: distM >= 1000 ? `${(distM/1000).toFixed(1)} กม.` : `${distM} ม.`,
                distM: distM,
                durSec: durSec,
                iconSvg: iconSvg,
                point: point
    ...
    window.toggleNavVoice = function() {
        ...
        const btn = document.getElementById('btnNavVoice');
        if (btn) {
            btn.classList.toggle('active', isVoiceGuidanceEnabled);
            btn.title = isVoiceGuidanceEnabled ? 'ปิดเสียงนำทาง' : 'เปิดเสียงนำทาง';
        }
    ...
    function speakThai(text) {
        if (!isVoiceGuidanceEnabled || !window.speechSynthesis || text === lastSpokenText) return;
        lastSpokenText = text;
        try {
    ...
    // ── Turn-by-Turn Steps Drawer UI ──
    window.toggleNavStepsDrawer = function() {
        const drawer = document.getElementById('gmapStepsDrawer');
        if (!drawer) return;
        const isVisible = drawer.style.display === 'flex';
        drawer.style.display = isVisible ? 'none' : 'flex';
        if (!isVisible) {
            renderStepsDrawer(activeNavRoute?.steps || []);
        }
    };

    function renderStepsDrawer(steps) {
        const container = document.getElementById('gmapStepsList');
        const summaryLabel = document.getElementById('stepsSummaryLabel');
        if (!container) return;

        if (!steps || steps.length === 0) {
            container.innerHTML = '<div style="padding:2rem;text-align:center;color:#94a3b8;">ยังไม่มีข้อมูลขั้นตอนการเดินทางบนถนนจริง</div>';
            return;
        }

        if (summaryLabel && activeNavRoute) {
            summaryLabel.textContent = `${activeNavRoute.name} • ${activeNavRoute.timeText} (${activeNavRoute.distKm} กม.)`;
        }

        let html = '';
        steps.forEach((s, idx) => {
            html += `
                <div class="gmap-step-card" onclick="highlightStepOnMap(${idx})">
                    <div class="gmap-step-icon">${s.iconSvg}</div>
                    <div class="gmap-step-body">
                        <div class="gmap-step-title">${s.instruction}</div>
                        <div class="gmap-step-meta">ระยะทาง ${s.distText}</div>
                    </div>
                </div>
            `;
        });
    ...
    window.highlightStepOnMap = function(stepIdx) {
        if (!activeNavRoute || !activeNavRoute.steps || !activeNavRoute.steps[stepIdx]) return;
        const step = activeNavRoute.steps[stepIdx];
        if (step.point) {
            try {
                map.flyTo(step.point, 18, { animate: true, duration: 0.8 });
            } catch(e) {}
        }
    };

    // ── GPS Movement Simulation (Demo Mode) ──
    let navSimInterval = null;
    let navSimIndex = 0;
    let isNavSimulating = false;

    window.toggleNavSimulation = function() {
        if (isNavSimulating) {
            stopNavSimulation();
        } else {
            startNavSimulation();
    ...
    function startNavSimulation() {
        if (!activeNavRoute || !activeNavRoute.points || activeNavRoute.points.length === 0) {
            window.alert('กรุณาเลือกเส้นทางก่อนเริ่มจำลองการเดินทาง');
            return;
        }
        isNavSimulating = true;
        navSimIndex = 0;
        const btn = document.getElementById('btnNavSim');
        if (btn) {
            btn.classList.add('active');
            btn.title = 'หยุดการจำลองการเดินทาง';
        }
        speakThai('เริ่มการจำลองการเดินทาง');

        navSimInterval = setInterval(() => {
            if (!isNavSimulating || !activeNavRoute || navSimIndex >= activeNavRoute.points.length) {
                stopNavSimulation();
                if (activeNavTarget) speakThai(`คุณมาถึง ${activeNavTarget.title} แล้ว`);
                return;
            }

            const point = activeNavRoute.points[navSimIndex];
            userCoords = point;
            setUserGpsMarker();
            updateNavigationPosition(point);
    ...
    function stopNavSimulation() {
        ...
        if (btn) {
            ...
            btn.title = 'จำลองการเดินทาง (Demo)';
        }
    }

    // ── Route Planner Modal Engine ──
    let plannerTravelMode = 'drive';

    window.openRoutePlannerModal = function() {
        const modal = document.getElementById('gmapRoutePlannerModal');
        if (!modal) return;
        modal.style.display = 'flex';
        populateRoutePlannerDropdowns();
    };

    window.closeRoutePlannerModal = function() {
        const modal = document.getElementById('gmapRoutePlannerModal');
    ...
    function populateRoutePlannerDropdowns() {
        ...
        let originOpts = '<option value="gps">📍 ตำแหน่งปัจจุบันของคุณ (GPS)</option>';
        let destOpts = '<option value="">🎯 เลือกสถานที่ หรือกิจกรรมปลายทาง...</option>';

        allLocations.forEach(loc => {
            const typeEmoji = loc.type === 'activity' ? '🎪' : (loc.type === 'job' ? '💼' : '🏛️');
            const opt = `<option value="${loc.type}_${loc.id}">${typeEmoji} ${loc.title} (${loc.location_name || ''})</option>`;
            originOpts += opt;
            destOpts += opt;
        });

        originSelect.innerHTML = originOpts;
        destSelect.innerHTML = destOpts;

        if (activeLocation) {
            destSelect.value = `${activeLocation.type}_${activeLocation.id}`;
        }
    }

    window.swapPlannerPoints = function() {
        const originSelect = document.getElementById('routeOriginSelect');
        const destSelect = document.getElementById('routeDestSelect');
        if (!originSelect || !destSelect) return;

        const tmp = originSelect.value;
        originSelect.value = destSelect.value || 'gps';
        destSelect.value = tmp;
    };

    window.selectPlannerMode = function(mode, btn) {
        plannerTravelMode = mode;
        currentTravelMode = mode;
        document.querySelectorAll('[data-planner-mode]').forEach(b => b.classList.remove('active'));
        if (btn) btn.classList.add('active');
    };

    window.executePlannedRoute = async function() {
        const originVal = document.getElementById('routeOriginSelect')?.value;
        const destVal = document.getElementById('routeDestSelect')?.value;

        if (!destVal) {
            window.alert('กรุณาเลือกจุดหมายปลายทาง');
            return;
        }

        // Resolve Destination
        const [destType, destId] = destVal.split('_');
        const destTarget = allLocations.find(l => String(l.id) === String(destId) && l.type === destType);
        if (!destTarget) {
            window.alert('ไม่พบสถานที่ปลายทางที่เลือก');
            return;
        }
        ...
        if (originVal === 'gps') {
            ...
            const [origType, origId] = originVal.split('_');
            const origTarget = allLocations.find(l => String(l.id) === String(origId) && l.type === origType);
            if (origTarget) {
                userCoords = [parseFloat(origTarget.lat), parseFloat(origTarget.lng)];
                setUserGpsMarker();
            }
        }

        closeRoutePlannerModal();
        openRouteSelector(destTarget);
    };
    ...
    function updateNavigationPosition(position) {
        ...
        if (remainingKm <= 0.03) {
            ...
            speakThai(`คุณมาถึง ${activeNavTarget.title} แล้ว`);
            return;
        }

        // Dynamic turn instruction & Thai voice prompts
        if (activeNavRoute?.steps && activeNavRoute.steps.length > 0) {
            let upcomingStep = null;
            for (let s of activeNavRoute.steps) {
                if (s.point) {
                    const dStep = calculateDistance(position[0], position[1], s.point[0], s.point[1]);
                    if (dStep < 0.12 && dStep > 0.01) {
                        upcomingStep = s;
                        break;
                    }
                }
            ...
            if (upcomingStep) {
                const nextText = document.getElementById('gmapNavNextText');
                if (nextText) nextText.textContent = upcomingStep.instruction;
                const turnIcon = document.getElementById('gmapNavTurnIcon');
                if (turnIcon && upcomingStep.iconSvg) turnIcon.innerHTML = upcomingStep.iconSvg;
                speakThai(`อีก ${upcomingStep.distText} ${upcomingStep.instruction}`);
            }
        }
    ...
    window.clearNavigationRoute = function() {
        stopNavSimulation();
        const stepsDrawer = document.getElementById('gmapStepsDrawer');
        if (stepsDrawer) stepsDrawer.style.display = 'none';
