...
<div style="display:flex; gap:0.5rem; border-bottom:1px solid #e2e8f0; margin-bottom:1.5rem;">
    ...
    <a href="{{ route('admin.settings.index', ['tab' => 'api-keys']) }}" style="padding:0.75rem 1.25rem; font-weight:600; font-size:0.9rem; text-decoration:none; border-bottom: 2px solid {{ $activeTab === 'api-keys' ? '#4f46e5' : 'transparent' }}; color: {{ $activeTab === 'api-keys' ? '#4f46e5' : '#64748b' }}; display:flex; align-items:center; gap:0.5rem;">
        ...
        API Keys & ความเป็นส่วนตัว
    </a>
</div>

@if($activeTab === 'api-keys')
    @include('admin.api-keys.index', ['tokens' => $tokens])
@else
