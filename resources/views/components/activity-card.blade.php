...
<div class="card act-card no-linkify" onclick="if(!event.target.closest('a, button')) window.location.href='{{ route('activities.show', $activity) }}'" style="cursor:pointer;">
    ...
    <div class="card-body">
        <div class="act-card-top">
            ...
            <h3 class="font-semi line-clamp-2 act-card-title">
                <a href="{{ route('activities.show', $activity) }}" style="color:inherit;text-decoration:none;" title="{{ $activity->title }}">{{ Str::limit($activity->title, 55, '...') }}</a>
            </h3>
