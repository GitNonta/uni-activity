...
export function EventsCard({ eventsData }) {
  ...
  const defaultEvents = [
    ...
    },
    {
      ...
      timestamp: 'August 5, 2026 at 11:26 AM',
    },
    {
      id: 3,
      type: 'failed',
      hash: '6ec09a0',
      message: 'fix(docker): add --no-scripts flag to composer dump-autoload in Dockerfile',
      detail: 'Exited with status 255 while running your code. Check your deploy logs for more information.',
      timestamp: 'August 5, 2026 at 3:46 AM',
    },
    {
      id: 4,
      type: 'started',
      ...
      message: 'fix(docker): add --no-scripts flag to composer dump-autoload in Dockerfile',
      detail: 'New commit via Auto-Deploy',
      timestamp: 'August 5, 2026 at 3:40 AM',
    },
    {
      id: 5,
      type: 'success',
      hash: '0e16a91',
      message: 'style(profile): remove background from SVG pencil edit icon',
      detail: 'Live - Deployed successfully from GitHub',
      timestamp: 'August 5, 2026 at 3:36 AM',
    },
    {
      id: 6,
      type: 'success',
      hash: 'f994766',
      message: 'style(profile): remove right card and add SVG pencil icon beside admin name',
      detail: 'Live - Deployed successfully from GitHub',
      timestamp: 'August 5, 2026 at 3:33 AM',
    }
  ];
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          <button type="button" style={{ background: '#1e293b', color: '#e2e8f0', border: '1px solid #334155', padding: '0.5rem 1rem', borderRadius: '8px', fontWeight: 600, fontSize: '0.85rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}>
            Connect <span>▾</span>
          </button>
          <button type="button" style={{ background: '#f8fafc', color: '#0f172a', border: 'none', padding: '0.5rem 1rem', borderRadius: '8px', fontWeight: 600, fontSize: '0.85rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}>
      ...
      <div style={{ marginBottom: '1rem' }}>
        <button type="button" style={{
          ...
          Filter events <span style={{ background: '#334155', color: '#fff', padding: '1px 6px', borderRadius: '10px', fontSize: '0.75rem' }}>{events.length}</span> <span>▾</span>
        </button>
      </div>
      ...
      <div style={{ border: '1px solid #1e293b', borderRadius: '10px', overflow: 'hidden', background: '#0f172a' }}>
        {events.map((ev) => (
          <div key={ev.id} style={{
            ...
            <div>
              {ev.type === 'failed' && (
                <button type="button" onClick={() => alert(`Initiating rollback process to stable commit for ${ev.hash}...`)} style={{
                  background: '#1e293b',
                  color: '#cbd5e1',
                  border: '1px solid #334155',
